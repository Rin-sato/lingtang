...
var src_default = new Proxy(
  {},
  {
    get(target, prop, receiver) {
      ...
      if (!_kv) {
        if (!process.env.ltdemo_KV_REST_API_URL || !process.env.ltdemo_KV_REST_API_TOKEN) {
          throw new Error(
        ...
        _kv = createClient({
          url: process.env.ltdemo_KV_REST_API_URL,
          token: process.env.ltdemo_KV_REST_API_TOKEN
        });
      }
  ...
  {},
  {
    get(target, prop) {
      if (!_kv) {
        if (!process.env.ltdemo_KV_REST_API_URL || !process.env.ltdemo_KV_REST_API_TOKEN) {
          throw new Error(
            "@vercel/kv: Missing required environment variables ltdemo_KV_REST_API_URL and ltdemo_KV_REST_API_TOKEN"
          );
        }
        _kv = createClient({
          url: process.env.ltdemo_KV_REST_API_URL,
